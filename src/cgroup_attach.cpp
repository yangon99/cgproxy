...
int attach(const string pid, const string cgroup_target)
{
        ...
        if (!dirExist(cgroup_target_path)) {
                if (mkdir(cgroup_target_path.c_str(),
                    ...
                    0) {
                        debug("created cgroup %s success",
                              cgroup_target.c_str());
                } else {
                        error("created cgroup %s failed, errno %d",
...
int write2procs(string pid, string procspath)
{
        ...
        if (!procs) {
                error("write %s to %s failed, maybe process %s live too short",
                      pid.c_str(), procspath.c_str(), pid.c_str());
                return_error;
        }
        return_success;
}
