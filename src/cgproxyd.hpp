...
bool loadExecsnoopLib()
{
        try {
                ...
                if (handle_dl == NULL) {
                        error("dlopen %s failed: %s", LIBEXECSNOOP_SO,
                              dlerror());
                        return false;
                }
                _startThread = reinterpret_cast<startThread_t *>(
                        dlsym(handle_dl, "startThread"));
...
class cgproxyd {
        ...
        int handle_pid(int pid)
        {
                unique_ptr<char[], decltype(&free)> path(
                        realpath(to_str("/proc/", pid, "/exe").c_str(), NULL),
                        &free);
                if (path == NULL) {
                ...
                if (find(v.begin(), v.end(), path.get()) != v.end()) {
                        ...
                        if (cg.empty()) {
                                debug("execsnoop: cgroup get failed, ignore: %d %s",
                                      pid, path.get());
                                return 0;
                        }
                        if (belongToCgroup(cg, config.cgroup_proxy_preserved) ||
                            belongToCgroup(cg,
                                           config.cgroup_noproxy_preserved)) {
                                info("execsnoop: already in preserverd cgroup, leave alone: %d %s",
                                     pid, path.get());
                                return 0;
                        }
                        if (!belongToCgroup(cg, config.cgroup_noproxy)) {
                                int res = attach(
                                        pid, config.cgroup_noproxy_preserved);
                                if (res == 0) {
                                        info("execsnoop; noproxy: %d %s", pid,
                                             path.get());
                                } else {
                                        info("execsnoop; noproxy failed: %d %s",
                                             pid, path.get());
                                }
                                return res;
                        }
                ...
                if (find(v.begin(), v.end(), path.get()) != v.end()) {
                        ...
                        if (!belongToCgroup(cg, config.cgroup_proxy)) {
                                int res = attach(pid,
                                                 config.cgroup_proxy_preserved);
                                if (res == 0) {
                                        info("execsnoop: proxied: %d %s", pid,
                                             path.get());
                                } else {
                                        info("execsnoop: proxied failed: %d %s",
                                             pid, path.get());
                                }
                                return res;
                        }
        ...
        int handle_msg(char *msg)
        {
                ...
                try {
                        ...
                        case MSG_TYPE_CONFIG_PATH:
                                status = config.loadFromFile(
                                        j.at("data").get<string>());
        ...
        void startSocketListeningThread()
        {
                ...
                future_status fstatus =
                        status_f.wait_for(chrono::seconds(THREAD_TIMEOUT));
        ...
        {
#ifdef BUIlD_EXECSNOOP_DL
                if (!EXECSNOOP::loadExecsnoopLib() ||
                    EXECSNOOP::_startThread == NULL) {
                ...
                future<void> status_f = status.get_future();
#ifdef BUIlD_EXECSNOOP_DL
                thread th(EXECSNOOP::_startThread, handle_pid_static,
                          move(status));
#else
                thread th(EXECSNOOP::startThread, handle_pid_static,
                          move(status));
#endif
        ...
        void processRunningProgram()
        {
                ...
                for (auto &path : config.program_noproxy)
                        for (auto &pid : bash_pidof(path)) {
                                string cg = getCgroup(pid);
                                if (cg.empty()) {
                                        debug("cgroup get failed, ignore: %d %s",
                                              pid, path.c_str());
                                        continue;
                                }
                                if (belongToCgroup(
                                    ...
                                    belongToCgroup(
                                            cg,
                                            config.cgroup_noproxy_preserved)) {
                                        info("already in preserverd cgroup, leave alone: %d %s",
                                             pid, path.c_str());
                                        continue;
                                }
                                if (!belongToCgroup(cg,
                                                    config.cgroup_noproxy)) {
                                        int status = attach(
                                                pid,
                                                config.cgroup_noproxy_preserved);
                                        if (status == 0)
                                                info("noproxy running process %d %s",
                                                     pid, path.c_str());
                                }
                        }
                        ...
                        for (auto &pid : bash_pidof(path)) {
                                ...
                                if (!belongToCgroup(cg, config.cgroup_proxy)) {
                                        int status = attach(
                                                ...
                                                config.cgroup_proxy_preserved);
                                        if (status == 0)
                                                info("proxied running process %d %s",
                                                     pid, path.c_str());
                                }
                        }
        }
    ...
    public:
        int start()
        {
                ...
                if (enable_execsnoop)
                        startExecsnoopThread();

                if (socketserver_thread.joinable())
                        socketserver_thread.join();
                if (execsnoop_thread.joinable())
                        execsnoop_thread.join();
...
void processArgs(const int argc, char *argv[])
{
        for (int i = 1; i < argc; i++) {
                ...
                if (strcmp(argv[i], "--execsnoop") == 0) {
                        enable_execsnoop = true;
                }
                if (argv[i][0] != '-') {
                        break;
                }
