...
#define merge(v)                                                        \
        {                                                               \
                v.erase(std::remove(v.begin(), v.end(), v##_preserved), \
                        v.end());                                       \
...
bool Config::validateJsonStr(const string js)
{
        json j = json::parse(js);
        bool status = true;
        const set<string> boolset = { "enable_gateway", "enable_dns",
                                      "enable_tcp",     "enable_udp",
        ...
        for (auto &[key, value] : j.items()) {
                if (key == "cgroup_proxy" || key == "cgroup_noproxy") {
                        if (value.is_string() && !validCgroup((string)value))
                                status = false;
                        // TODO what if vector<int> etc.
                        if (value.is_array() &&
                            !validCgroup((vector<string>)value))
                                status = false;
...
void Config::print_summary()
{
        ...
        info("table: %d, fwmark: %d, mark_newin: %d", table, fwmark,
             mark_newin);
}
