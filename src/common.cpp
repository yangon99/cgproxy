...
{
        vector<int> pids;
        unique_ptr<FILE, decltype(&pclose)> fp(
                popen(to_str("pidof ", path).c_str(), "r"), &pclose);
        if (!fp)
                return pids;
        int pid;
        while (fscanf(fp.get(), "%d", &pid) != EOF) {
                pids.push_back(pid);
        }
        return pids;
}
...
string bash_which(const string &name)
{
        stringstream buffer;
        unique_ptr<FILE, decltype(&pclose)> fp(
                popen(to_str("which ", name).c_str(), "r"), &pclose);
        if (!fp)
                return "";
        char buf[READ_SIZE_MAX];
        while (fgets(buf, READ_SIZE_MAX, fp.get()) != NULL) {
                buffer << buf;
        }
        string s = buffer.str();
        if (!s.empty())
...
string bash_readlink(const string &path)
{
        stringstream buffer;
        unique_ptr<FILE, decltype(&pclose)> fp(
                popen(to_str("readlink -e ", path).c_str(), "r"), &pclose);
        if (!fp)
                return "";
        char buf[READ_SIZE_MAX];
        while (fgets(buf, READ_SIZE_MAX, fp.get()) != NULL) {
...
string getCgroup(const string &pid)
{
        ...
        while (ifs.good() && getline(ifs, line)) {
                ...
                if (line[0] == '0') {
                        ...
                        debug("get cgroup of %s: %s", pid.c_str(),
                              cgroup.c_str());
                        break;
                }
        }
